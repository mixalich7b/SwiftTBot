...
extension TBError: CustomStringConvertible {
    public var description: String { get {
        switch self {
            ...
            case .networkError(_, let error): return "Network error: \(error?.localizedDescription ?? "unknown")"
            case .wrongResponseData(let responseData, _, _): return "Wrong response: \(NSString(data: responseData, encoding: String.Encoding.utf8.rawValue) ?? "couldn't read")"
            case .protocolError(let description): return "Protocol error: \(description ?? "unknown")"
